      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          if (delayMs > 0) {
            setTimeout(function () {
              el.classList.add(ANIMATION_CLASS);
              el.classList.remove(OBSERVE_CLASS);
            }, delayMs);
          } else {
            el.classList.add(ANIMATION_CLASS);
            el.classList.remove(OBSERVE_CLASS);
    ...
    elements.forEach(function (el, index) {
      ...
      if (!el.getAttribute('data-delay') && !el.getAttribute('data-animate-delay')) {
        const siblings = el.parentNode ? Array.from(el.parentNode.children).filter(function (child) {
          return child.matches(animatableSelectors.join(', '));
        }) : [];
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "PrePeak משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'e3b10b4e-b848-44c6-b433-0283335c695a',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'e3b10b4e-b848-44c6-b433-0283335c695a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'e3b10b4e-b848-44c6-b433-0283335c695a';
